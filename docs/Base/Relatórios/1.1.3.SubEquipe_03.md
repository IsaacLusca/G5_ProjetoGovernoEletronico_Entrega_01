...
#### 6.3 Validação dos rascunhos produzidos por IA generativa

Como experimento de apoio, as especificações textuais foram enviadas ao NotebookLM para geração de imagens. Os dois resultados foram avaliados visual e semanticamente antes de qualquer inclusão no relatório:

| Rascunho | Aspectos adequados | Problemas encontrados | Decisão |
| --- | --- | --- | --- |
| Busca de informações públicas | Separação visual em raias, tarefas legíveis e gateway exclusivo | Ausência de fluxos de sequência entre vários elementos, símbolos isolados e caminhos “Sim/Não” sem ligação completa | Reprovado como BPMN |
| Registro de pedido | Raias, tarefas, decisões e parte das conexões foram representadas | Saídas “Sim/Não” invertidas nos gateways de anexo e preservação da identidade; interação com GOV.BR modelada de forma inconsistente | Reprovado como BPMN |

Os rascunhos não serão utilizados nos espaços reservados acima. A decisão evidencia que boa aparência não garante correção sintática ou semântica. A versão final deverá ser reconstruída em uma ferramenta de modelagem compatível com BPMN 2.0 e conferida elemento a elemento com a especificação da OMG e com a tabela de rastreabilidade deste relatório. O Rich Picture eventualmente gerado por IA poderá ser utilizado apenas como visão contextual complementar, sem substituir o modelo BPMN exigido.

#### 6.4 Rich Picture complementar

O Rich Picture abaixo sintetiza os participantes, fontes de informação, interações e tensões de privacidade e usabilidade recuperadas durante a engenharia reversa. Ele foi validado apenas como visão contextual e não substitui os espaços reservados para os modelos BPMN.

![Rich Picture do ecossistema de transparência e acesso à informação](./assets/bpmn/rich-picture-ecossistema-transparencia.jpg)
